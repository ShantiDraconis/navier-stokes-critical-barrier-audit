...
/--
Abstract Gronwall step. Unlike the previous version, it consumes the exact
balance, flexible bound and time-control certificate produced on this path.
-/
structure GronwallClosure
    (ν : ℝ) (d : EnstrophyData) (yPrime : ℝ → ℝ)
    (UniformEnstrophy : Prop) where
  close : EnstrophyBalance ν d yPrime →
    ∀ (θ : ℝ) (a : ℝ → ℝ),
      FlexibleStretchingBound ν θ a d →
      L1TimeControl a →
      UniformEnstrophy
...
Every downstream certificate must be generated by the connected DAG.
-/
theorem conditional_continuation
    ...
    (gr : GronwallClosure ν d yPrime UniformEnstrophy)
    (reconstruct : EnstrophyToCriticalBound UniformEnstrophy CriticalBound)
    (ess : ESSEndpoint CriticalBound Continuation)
    (hNS : ActualNS) :
    Continuation := by
  have q : DynamicQBound := nsToQ.bridge hNS
  have kwc : KernelWeightedCoherence := qToKernel.bridge q
  have hflex : FlexibleStretchingBound ν kt.θ kt.a d :=
    kt.stretching kwc tailCert
  have htime : L1TimeControl kt.a :=
    kt.timeControl kwc tailCert
  have hEns : UniformEnstrophy :=
    gr.close hbal kt.θ kt.a hflex htime
