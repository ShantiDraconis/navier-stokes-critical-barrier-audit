...
theorem xiEps_norm_le_one_for_mean
  ...
  have hsq : (magEps omega eps x) ^ 2 = ‖omega x‖ ^ 2 + eps ^ 2 := by
    unfold magEps
    rw [Real.sq_sqrt]
    positivity
  have hsqrt : ‖omega x‖ ≤ magEps omega eps x := by
    have hmnonneg : 0 ≤ magEps omega eps x := magEps_nonneg omega eps x
    have hnormnonneg : 0 ≤ ‖omega x‖ := norm_nonneg _
...
theorem ballMeanBound
  ...
  have htop : volume (ball c r) ≠ (⊤ : ℝ≥0∞) := measure_ball_lt_top.ne
